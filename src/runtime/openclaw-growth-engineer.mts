...
function buildIssueDraft(signal, matchedFiles, titlePrefix, activeCadences: any[] = []): any {
...
function normalizeCadencePlan(payload) {
  ...
  return rawCadences
    ...
    .map((cadence) => ({
      ...
      sourcePriorities: toStringArray(cadence.sourcePriorities || cadence.source_priorities).map((value) =>
        value.toLowerCase(),
      ),
...
function isCriticalSignal(signal) {
  ...
  if (signal.priority === 'high') return true;
  if (signal.area === 'crash') return true;
  ...
  if (/(conversion|purchase|revenue|trial|checkout|paywall|user|active|activation)/i.test(metric)) {
    ...
    if (Number.isFinite(signal.deltaPercent) && signal.deltaPercent <= -20) return true;
...
function cadenceSignalScore(signal, cadences) {
  ...
  for (const cadence of cadences) {
    if (cadence.focusAreas.includes(signal.area)) score += 8;
    if (cadence.sourcePriorities.includes(String(signal.source).toLowerCase())) score += 6;
    if (cadence.criticalOnly && isCriticalSignal(signal)) score += 12;
...
function applyCadencePlan(signals, cadences) {
  if (!cadences.length) return signals;
...
async function githubApiRequest(url, token, options: RequestInit = {}) {
...
async function main() {
  ...
  const rankedSignals = dedupeSignals(
    rankSignals([
  ...
  if (args.createPullRequests && !args.allowProposalPullRequests) {
    throw new Error(
      '--create-pull-requests only creates proposal-only markdown PRs. Use --allow-proposal-pull-requests when explicitly requested, or have OpenClaw implement code changes in the target repo instead.',
    );
  }
